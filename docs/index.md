...
敏捷开关是一个开源的feature flags解决方案。他允许团队中任意指定人员使用无代码操作，细粒度的控制一个产品功能的发布、回退。细粒度，特指可以将产品的功能在任意时间实时的发布给任意用户（活用户群），同理也可以将发布出去的功能进行细粒度的秒级回退。随着feature flags系统的进化，更多的团队使用它进行更加灵活迅速的AB测试。
